...
func particleThatRemainsClosest(w particle.World) (closest *particle.Particle) {
	...
	for {
		...
		if iteration%100 == 0 {
			closest = w.FindClosestOrigin()

			fmt.Printf("iteration %v: closest = %v\n", iteration, closest)
			if iteration >= 500 {
				return
			}
		}
...
func particlesRemainingAfterCollisions(w particle.World) int {
	iteration := 0
	for {
		w.Tick()
		w.RemoveCollisions()
		...
		if iteration%10 == 0 {
			fmt.Printf("iteration %v: particles remaining %v\n", iteration, len(w))
			if iteration >= 100 {
				return len(w)
			}
		}
...
var input = `p=<2366,784,-597>, v=<-12,-41,50>, a=<-5,1,-2>
